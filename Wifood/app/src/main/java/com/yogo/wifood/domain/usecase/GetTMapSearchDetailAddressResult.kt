...
class GetTMapSearchDetailAddressResult @Inject constructor(
    private val repository: WifoodRepository
) {
    operator fun invoke(keyword: String): LiveData<MutableList<TMapSearch>> {
        val tmapSearchPlace = MutableLiveData<MutableList<TMapSearch>>()
        repository.getTMapSearchDetailAddressResult(keyword).observeForever {
